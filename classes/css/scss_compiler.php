...
class scss_compiler {
    ...
    protected $formatter = "\\core\\classes\\css\\scss_formatter_nested";
    public $file_extension = 'scss';
    protected $source;

    public function compile($code = null, $name=null) {
        if($code == null) {
            $code = $this->source;
        }
        $this->indentLevel = -1;
    ...
    public function add_file($file_name) {
        $this->addImportPath(pathinfo($file_name, PATHINFO_DIRNAME));
...
class scss_parser {
    ...
    static protected function makeOperatorStr($operators) {
        return '('.implode('|', array_map(array('\\core\\classes\\css\\scss_parser','preg_quote'),
            $operators)).')';
    }
     ...
     *
     * Consider the function scss_compiler::keyword(). (All parse functions are
     * structured the same.)
     *
     ...
     *
     * All of these parse functions are powered by scss_compiler::match(), which behaves
     ...
     * avoid this, scss_compiler::seek() is used to remember and set buffer positions.
     *
    ...
    protected function selectorSingle(&$out) {
        ...
        while (true) {
            ...
            // self
            if ($this->literal("&", false)) {
                $parts[] = scss_compiler::$selfSelector;
                continue;
            }
...
class scss_server {
     ...
     * @param \scss_compiler|null $scss     SCSS compiler instance
     */
